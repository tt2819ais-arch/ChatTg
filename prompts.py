...
def _rules(cfg: AppConfig) -> str:
    ...
    return (
        ...
        "проще — скажи проще.\n"
        f"- ОЧЕНЬ КОРОТКО: 1-2 коротких предложения, максимум ~{cfg.max_message_chars} "
        "символов. Никаких длинных полотен и списков. Одна мысль за ход.\n"
        "- Не повторяй то, что уже сказали — добавляй своё.\n"
        "- Не здоровайся каждый раз и не подписывайся (имя и так видно). На «ты».\n"
        "- ТЕМА: обсуждай ТОЛЬКО последнюю тему, которую задал Босс. Если Босс "
        "сменил тему — сразу забудь прежнюю и говори про новую, не возвращайся к "
        "старой.\n"
        "- Отвечай и реагируй не только Боссу, но и другим людям в чате — но "
        "приоритет всегда у Босса.\n"
...
# ── Детектор смены темы: Босс заговорил про другое → забыть старое ────────────

TOPIC_SHIFT_SYSTEM = (
    "Ты определяешь, СМЕНИЛ ли Босс тему обсуждения. Дан предыдущий диалог и новое "
    "сообщение Босса. Ответь СТРОГО одним JSON без markdown: "
    '{"new_topic": true|false}.\n'
    "new_topic=true — если Босс заговорил про ДРУГУЮ идею/проект/задачу, не "
    "связанную с предыдущим обсуждением.\n"
    "new_topic=false — если это уточнение, ответ на вопрос, развитие той же темы, "
    "короткая реплика, команда «погнали» или эмоция."
)


def topic_shift_user_prompt(prev: List[dict], new_text: str, cfg: AppConfig) -> str:
    return (
        "Предыдущее обсуждение:\n"
        f"{render_transcript(prev, cfg, limit=12)}\n\n"
        f"Новое сообщение Босса: «{new_text}»\n\n"
        "Это новая тема? Ответь JSON: {\"new_topic\": true|false}."
    )


def parse_topic_shift(raw: str) -> bool:
    if not raw:
        return False
    text = raw.strip()
    start, end = text.find("{"), text.rfind("}")
    if start == -1 or end == -1 or end <= start:
        return "true" in text.lower() and "new_topic" in text.lower()
    try:
        data = json.loads(text[start : end + 1])
    except Exception:  # noqa: BLE001
        return False
    return bool(data.get("new_topic", False))
